...
Strategy simulation uses deterministic rules while analysis, optimization and development use AI agents with human input (so non-deterministic analysis with planning/reasoning and deep research). 
...
- experiment definition including goals (e.g., return exceeds market index CAGR over the same time period) and constraints on  parameters, rules or outcome (e.g., realized drawdown no more than 20%)
- strategy definition and strategy parameters (needed in order to reproduce strategy backtests later)
- experiment results, including metrics generated by simulations and experiment conclusion. 
...
Run 
`uv run python backfire/visualize_signal.py --signal my_signal.yaml --signal.name=FTDSignal --signal.ftd_min_gain=0.02 --signal.ftd_max_days=30`
